...
class VideoSearchApp:
    def __init__(self, root):
        ...
        self.known_face_encodings = [] # Now a list to store all detected faces
        self.running_search = False
        self.tolerance_var = tk.StringVar(value="0.5") # Default tolerance for matching
        self.frame_skip_var = tk.StringVar(value="5") # Default frame skip for performance
    ...
    def create_widgets(self):
        ...
        # Frame for settings (tolerance and frame skip)
        settings_frame = tk.Frame(main_frame)
        settings_frame.pack(fill="x", pady=5)

        tk.Label(settings_frame, text="Matching Tolerance (0.0-1.0):").pack(side="left", padx=(0, 5))
        self.tolerance_entry = tk.Entry(settings_frame, textvariable=self.tolerance_var, width=5)
        self.tolerance_entry.pack(side="left")
        
        tk.Label(settings_frame, text="Frame Skip (1-100+):").pack(side="left", padx=(10, 5))
        self.frame_skip_entry = tk.Entry(settings_frame, textvariable=self.frame_skip_var, width=5)
        self.frame_skip_entry.pack(side="left")
    ...
    def select_image(self):
        ...
        if file_path:
            ...
            self.known_face_encodings = self.get_face_encodings(file_path)
    ...
    def get_face_encodings(self, image_path):
        """
        Loads an image and gets all its face encodings.
        """
        try:
    ...
    def find_person_in_videos(self):
        ...
        try:
            ...
            self.log_message("Invalid tolerance or frame skip value. Please enter numbers.")
            self.stop_search()
            return
        ...
        # Create a single folder for all screenshots
        output_folder = "screenshots"
        os.makedirs(output_folder, exist_ok=True)
        self.log_message(f"All screenshots will be saved to: {os.path.abspath(output_folder)}")
        ...
        for video_file in video_files:
            ...
            frame_count = 0
            self.progress_var.set(0) # Reset progress for new video
            ...
            while video_capture.isOpened():
                ...
                frame_count += 1
                self.progress_var.set(frame_count)
                ...
                for face_encoding in face_encodings:
                    ...
                    if True in matches:
                        # Save the matching frame as a screenshot
                        screenshot_filename = f"{video_name}_match_frame_{frame_count}.jpg"
