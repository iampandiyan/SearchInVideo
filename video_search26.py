...
# === ADD FACE TO REFERENCE DYNAMICALLY ===
def add_face_to_reference(cropped_face, ref_embeddings, ref_output):
    try:
        processed_face = preprocess_face(cropped_face)
        faces = app.get(processed_face)
        if not faces:
            faces = app.get(cropped_face)
        if faces:
            new_emb = faces[0].normed_embedding
            ref_embeddings.append(new_emb)
            save_path = os.path.join(ref_output, f"dynamic_ref_{len(ref_embeddings):03d}.jpg")
            cv2.imwrite(save_path, cropped_face)
            print(f"✅ Added new reference face: {save_path}")
        else:
            print("⚠️ Could not extract embedding for new reference face.")
    except Exception as e:
        print(f"⚠️ Failed to add face to reference: {e}")
...
def process_videos():
    ...
    ref_embeddings = load_reference_embeddings(ref_folder, ref_output)
    ...
    for i, video_name in enumerate(video_files, start=1):
        ...
        while cap.isOpened() and frame_count < max_frames_val:
            ...
            for idx, face in enumerate(faces):
                ...
                # Retry with preprocessing if too small/blurry
                if cropped_face.shape[0] < 50 or cropped_face.shape[1] < 50 or is_blurry(cropped_face):
                    retry_face = preprocess_face(cropped_face)
                    if retry_face.shape[0] >= 50 and retry_face.shape[1] >= 50 and not is_blurry(retry_face):
                        cropped_face = retry_face
                    else:
                ...
                emb = face.normed_embedding
                min_distance = min(np.linalg.norm(emb - ref_emb) for ref_emb in ref_embeddings)

                if min_distance < threshold:
                    match_path = os.path.join(matched_faces_folder, f"{video_name}_frame{frame_count:04d}_face{idx}_match.jpg")
                    cv2.imwrite(match_path, cropped_face)
                    print(f"✅ Match found (distance={min_distance:.4f}) — saved {match_path}")

                    # Add to reference list immediately
                    add_face_to_reference(cropped_face, ref_embeddings, ref_output)
                    ...
                    if processed_faces:
                        ...
                        processed_distance = min(np.linalg.norm(processed_emb - ref_emb) for ref_emb in ref_embeddings)
                        if processed_distance < threshold:
                            match_path = os.path.join(matched_faces_folder, f"{video_name}_frame{frame_count:04d}_face{idx}_match_fallback.jpg")
                            cv2.imwrite(match_path, processed_face)
                            print(f"✅ Match found after preprocessing (distance={processed_distance:.4f}) — saved {match_path}")

                            # Add to reference list immediately
                            add_face_to_reference(processed_face, ref_embeddings, ref_output)

                            match_found = True
                            break

                    # Still no match → save to not_matched
