    ...
    for file in sorted(os.listdir(folder)):
        if file.lower().endswith((".jpg", ".png", ".jpeg")):
            ...
            try:
                ...
                continue
    # ensure the scroll region gets recalculated
    container.update_idletasks()

def refresh_face_tabs():
    """Refresh all 3 face tabs from the current output folders."""
    ...
    try:
        ...
        for idx_video, video_name in enumerate(sorted(video_files), start=1):
            ...
            if not cap.isOpened():
                ...
                # refresh tabs in case something was written
            ...
            # LIVE: refresh tabs after each video so thumbnails appear immediately
            safe_after(0, refresh_face_tabs)

        # After loop, ensure tabs are populated (final)
        safe_after(0, refresh_face_tabs)
...
# Start with empty tabs (they’ll fill during/after processing)
clear_container(matched_faces_frame_inner)
clear_container(unmatched_faces_frame_inner)
clear_container(skipped_faces_frame_inner)
