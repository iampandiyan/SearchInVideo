...
app = FaceAnalysis(model_path=get_resource_path("insightface/models/buffalo_l"))
...
def toggle_selection(widget, img_path):
    ...
    if img_path in selected_images:
        ...
        widget.config(relief="solid", borderwidth=2, bg="#b3e5fc") # Light blue highlight
...
def open_preview(img_path):
    ...
    try:
        ...
        lbl = Label(win, image=tkimg, bg="#e3f2fd")
        lbl.image = tkimg
    ...
    for file in sorted(os.listdir(folder)):
        if file.lower().endswith((".jpg", ".png", ".jpeg")):
            ...
            try:
                ...
                btn = Button(container, image=img_tk, relief="flat", borderwidth=1, bg="#e3f2fd")
                btn.image = img_tk
                ...
                if img_path in selected_images:
                    btn.config(relief="solid", borderwidth=2, bg="#b3e5fc")
...
def process_videos():
    ...
    try:
        ...
        safe_after(0, reset_btn.config, {"state": DISABLED}) # Disable reset during processing
        ...
        for idx_video, video_name in enumerate(sorted(video_files), start=1):
            ...
            # --- NEW: Temporary storage for unmatched faces
            temp_unmatched_faces = []
            ...
            while cap.isOpened() and frame_count < max_fr:
                ...
                for fi, face in enumerate(faces):
                    ...
                    if dist < dist_thr:
                        ...
                        cv2.imwrite(outp, candidate)
                        # --- NEW: Add matched embedding to reference list
                        ref_embeddings.append(face.normed_embedding)
                    else:
                        # --- NEW: Store unmatched face temporarily
                        outp = os.path.join(output_folder.get(), "not_matched_faces", f"{os.path.splitext(video_name)[0]}_f{frame_count:06d}_nomatch.jpg")
                        temp_unmatched_faces.append((candidate, outp))
                
                # --- NEW: If a match is found, break the inner loop (processing faces in the current frame)
                if match_found: break
            
            cap.release()
            safe_after(0, processed_videos_var.set, processed_videos_var.get() + 1)
            
            # --- NEW: Finalize face placement based on match_found flag
            if match_found:
                # Move all temporarily stored unmatched faces to the 'moved_faces' folder
                for face_img, old_path in temp_unmatched_faces:
                    new_path = old_path.replace("not_matched_faces", "moved_faces")
                    cv2.imwrite(new_path, face_img)
                    log(f"📦 Moved an unmatched face for a matched video to '{os.path.basename(new_path)}'")
                ...
                # If no match was found, save the temporary faces to their original destination
                for face_img, old_path in temp_unmatched_faces:
                    cv2.imwrite(old_path, face_img)
        ...
        safe_after(0, reset_btn.config, {"state": NORMAL}) # Enable reset after processing is done
        log("\n🏁 Done.\n")
        safe_after(0, update_button_states) # Re-check button state after finishing

# This new function handles the confirmation pop-up
def confirm_and_start():
    base_folder_path = output_folder.get()
    
    if not os.path.exists(base_folder_path):
        os.makedirs(base_folder_path)

    # Check if the folder is not empty (ignoring hidden files)
    if any(os.listdir(base_folder_path)):
        # Show a confirmation pop-up
        response = messagebox.askyesno(
            "Confirm Overwrite",
            "Previous run data in the base folder will be deleted and new run data will be created. Shall we proceed?"
        )
        if not response:
            log("🚫 Operation cancelled by user.")
            return

        # If YES is clicked, delete all contents
        log("🗑️ Deleting all contents in the base folder...")
        for item in os.listdir(base_folder_path):
            ...
            try:
                ...
                log(f"⚠️ Error deleting {item_path}: {e}")
        log("✅ Base folder cleared.")
        
    # Start the processing thread regardless of whether it was empty or cleared
    threading.Thread(target=process_videos, daemon=True).start()


def start_processing():
    # Clear the console and image tabs
    console_text.configure(state="normal")
    ...
    # Launch the confirmation check in a new thread to avoid freezing the GUI
    threading.Thread(target=confirm_and_start, daemon=True).start()
...
def stop_processing():
    ...
    safe_after(0, reset_btn.config, {"state": NORMAL}) # Enable reset when stopped manually
...
# ========= GUI SETUP =========
root = Tk()
root.title("Face Match Video Search And Copy")
root.geometry("1060x780")
root.configure(bg="#e3f2fd") # Light blue theme background

# --- FEATURE 1: Create separate popup menus ---
popup_menu = Menu(root, tearoff=0, bg="#ffffff", fg="#263238")
popup_menu.add_command(label="Move Selected", command=move_selected_files)
popup_menu_matched = Menu(root, tearoff=0, bg="#ffffff", fg="#263238")
...
style.configure("TNotebook.Tab", background="#bbdefb", foreground="#263238", font=('Helvetica', 10, 'bold'), padding=[10, 5], relief="flat", focuscolor="#bbdefb")
style.map("TNotebook.Tab", background=[("selected", "#ffffff"), ("!selected", "#bbdefb")], foreground=[("selected", "#263238"), ("!selected", "#263238")])
style.configure("TFrame", background="#e3f2fd")
style.configure("TLabel", background="#e3f2fd", foreground="#263238")
style.configure("TEntry", fieldbackground="#ffffff", foreground="#263238", borderwidth=1, relief="solid")
style.configure("TButton", background="#81d4fa", foreground="#000000", font=('Helvetica', 10, 'bold'), borderwidth=1, relief="solid", focuscolor="#81d4fa")
style.map("TButton", background=[("active", "#4fc3f7"), ("disabled", "#ffffff")], foreground=[("disabled", "#263238")])
style.configure("TProgressbar", thickness=15, troughcolor="#bbdefb", bordercolor="#4fc3f7", background="#039be5")
...
max_frames = StringVar(value="500")
...
tab_names = ["Setup & Run", "Reference Faces", "Matched Faces", "Unmatched Faces", "Skipped Faces", "Moved Faces"]
...
def rowpair(parent, r, label, var, browse=False, cmd=None):
    ...
    if browse: ttk.Button(parent, text="Browse", command=cmd).grid(row=r, column=2, padx=6)

tab_stats = tabs["Setup & Run"]
...
buttons_frame = ttk.Frame(tab_stats)
buttons_frame.grid(row=7, column=0, columnspan=3, pady=10, sticky="w")
start_btn = ttk.Button(buttons_frame, text="Start Processing", command=start_processing, width=18)
stop_btn = ttk.Button(buttons_frame, text="Stop Processing", command=stop_processing, width=16, state="disabled")
reset_btn = ttk.Button(buttons_frame, text="Reset", command=reset_all, width=10)
refresh_btn = ttk.Button(buttons_frame, text="Refresh Thumbnails", command=refresh_face_tabs, width=18)
...
for i, (label, var) in enumerate([("Total Videos:", total_videos_var), ("Processed:", processed_videos_var),
    ...
    ttk.Label(stats_frame, text=label).grid(row=i, column=0, sticky="w", padx=6)
    ttk.Label(stats_frame, textvariable=var).grid(row=i, column=1, sticky="w", padx=6)

ttk.Label(tab_stats, text="Console Log:").grid(row=10, column=0, sticky="w", padx=6, pady=(8, 2))
console_text = scrolledtext.ScrolledText(tab_stats, height=10, state="disabled", wrap="word", bg="#ffffff", fg="#263238", insertbackground="#263238", borderwidth=1, relief="solid")
...
def create_scrollable_tab(tab):
    canvas = Canvas(tab, borderwidth=0, bg="#e3f2fd", highlightthickness=0)
    scrollbar = Scrollbar(tab, orient="vertical", command=canvas.yview)
    scroll_frame = Frame(canvas, bg="#e3f2fd")
