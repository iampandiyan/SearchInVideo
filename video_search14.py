...
def load_reference_embeddings(folder_path, save_folder):
    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith((".jpg", ".png", ".jpeg")):
            ...
            face_img = faces[0]["face"]  # First detected face
...
while cap.isOpened() and frame_count < MAX_FRAMES:
    ...
    for face in results:
        ...
        print(f"   ✅ Distance to TRUE reference: {distance_true:.2f}")
        print(f"   ❌ Distance to FALSE reference: {distance_false:.2f}")

        # === MATCHING LOGIC ===
        if distance_true < DISTANCE_THRESHOLD and distance_true < distance_false:
            label = f"True Match ({distance_true:.2f})"
            color = (0, 255, 0)
        else:
